...
Раздел «Логи» разделён на вкладки журналов компонентов, запусков парсеров и доставки сообщений.
Записи можно просматривать списком или в консольном виде: переключатель вида сохраняет
фильтры, а вкладки потоков выбирают отдельный компонент. В консоли новые записи находятся внизу;
при прокрутке к старым строкам автоматическое обновление не перемещает просмотр вниз.
Владельцу и оператору доступны журналы бота, административного API, сайта,
...
Владелец может управлять двумя независимыми ограничениями в разделе «Пользователи»:

- «Заблокировать бота» отключает обработку команд, кнопок и inline-запросов пользователя, а также исходящие уведомления и напоминания.
- «Запретить обращения» отключает горячую линию, `/report` и отправку уже открытой формы. Просмотр расписания и остальные настройки доступны.

Кнопки «Разблокировать бота» и «Разрешить обращения» снимают соответствующее ограничение. Изменения сохраняются на сервере и действуют без перезапуска процессов. Уже переданное Telegram сообщение отозвать таким переключением нельзя. Ранее принятые обращения остаются в админке для рассмотрения.

Каждое изменение записывается в аудит вместе с прежними и новыми значениями. API: `PATCH /api/users/{id}/restrictions`, с действующей сессией владельца и CSRF-токеном. Тело содержит `bot_blocked` и/или `support_blocked` типа boolean; пропущенное поле сохраняет своё значение. Ответ возвращает оба текущих значения. Поля также доступны в `GET /api/users`. Для обновления требуется миграция 054.
