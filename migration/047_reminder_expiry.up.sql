ALTER TABLE bot_outbox ADD COLUMN expires_at TIMESTAMPTZ, ADD COLUMN reminder_context JSONB NOT NULL DEFAULT '{}'::JSONB;
UPDATE bot_outbox SET status='cancelled', claim_token='', lease_expires_at=NULL,
    cancel_requested_at=NULL, cancel_reason='', updated_at=NOW(), last_error='legacy reminder requires regeneration'
WHERE kind='lesson_reminder' AND status='pending';
CREATE INDEX bot_outbox_reminder_expiry ON bot_outbox(expires_at) WHERE kind='lesson_reminder' AND status='pending';
