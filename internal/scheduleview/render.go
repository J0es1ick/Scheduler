...
type Request struct {
	TimeSlots               []domain.LessonTimeSlot
	University              string
	Group                   string
	From                    time.Time
	Days                    int
...
const (
	maxConcurrentPNGRenders = 4
	maxRenderDays           = 14
	maxLessonsPerRenderDay  = 32
	maxLessonsPerRender     = 224
	maxRenderPixels         = 25_000_000
)

var ErrRenderLimit = errors.New("schedule exceeds visual rendering limits")

var pngRenderSlots = make(chan struct{}, maxConcurrentPNGRenders)

func RenderPNG(request Request) ([]byte, error) {
	return RenderPNGContext(context.Background(), request)
}

func RenderPNGContext(ctx context.Context, request Request) ([]byte, error) {
	if ctx == nil {
		ctx = context.Background()
	}
	select {
	case pngRenderSlots <- struct{}{}:
	case <-ctx.Done():
		return nil, ctx.Err()
	}
	...
	if request.Days > maxRenderDays {
		return nil, fmt.Errorf("%w: %d days", ErrRenderLimit, request.Days)
	}
	days := completeDays(request)
	if err := validateRenderSize(request, days); err != nil {
		return nil, err
	}
...
func validateRenderSize(request Request, days []Day) error {
	total := 0
	for _, day := range days {
		if len(day.Lessons) > maxLessonsPerRenderDay {
			return fmt.Errorf("%w: %s has %d lessons", ErrRenderLimit, day.Date.Format(time.DateOnly), len(day.Lessons))
		}
		total += len(day.Lessons)
	}
	if total > maxLessonsPerRender {
		return fmt.Errorf("%w: %d lessons", ErrRenderLimit, total)
	}
	width, height := renderDimensions(request, days)
	if width <= 0 || height <= 0 || int64(width)*int64(height) > maxRenderPixels {
		return fmt.Errorf("%w: %dx%d pixels", ErrRenderLimit, width, height)
	}
	return nil
}
...
func renderDimensions(request Request, days []Day) (int, int) {
	if request.Days == 1 {
		return 1100, 213 + max(1, len(dayEntries(request, days[0])))*145
	}
	height := 150 + 45
	weekCount := (len(days) + 6) / 7
	height += max(0, weekCount-1) * 34
	for index := 0; index < weekCount; index++ {
		from := index * 7
		to := min(from+7, len(days))
		height += weekHeight(days[from:to], request.NormalizeResearchBlocks, request.TimeSlots)
	}
	return 1900, height
}
...
func renderDay(request Request, day Day, faces *faces) *image.RGBA {
	...
	entries := dayEntries(request, day)
	height := 213 + max(1, len(entries))*lessonHeight
	...
	if len(entries) == 0 {
		metrics := faces.header.Metrics()
		baseline := panel.Min.Y + (panel.Dy()-(metrics.Ascent+metrics.Descent).Ceil())/2 + metrics.Ascent.Ceil()
	...
	for _, entry := range entries {
		...
		if entry.lesson == nil {
			drawCentered(canvas, faces.header, textColor, timeRect.Min.X, timeRect.Max.X, timeRect.Min.Y+52, entry.slot.start)
			drawCentered(canvas, faces.body, mutedColor, timeRect.Min.X, timeRect.Max.X, timeRect.Min.Y+82, entry.slot.end)
			label := emptySlotLabel(day.Lessons, entry.slot, false)
			if label == "Окно" {
				label = "Окно — занятий нет"
			} else if label == "" {
				label = "Занятий нет"
			}
			drawCentered(canvas, faces.header, mutedColor, lessonRect.Min.X, lessonRect.Max.X, lessonRect.Min.Y+70, label)
			y += lessonHeight
			continue
		}
		lesson := *entry.lesson
		...
		drawText(clipped, faces.body, mutedColor, lessonRect.Min.X+18, lessonRect.Min.Y+27,
		...
		details := visualLessonDetailsWithOptions(lesson, slot, request.ShowGroupNames)
		if details != "" {
...
func renderWeeks(request Request, days []Day, faces *faces) *image.RGBA {
	...
	for index := 0; index < weekCount; index++ {
		from := index * 7
		to := min(from+7, len(days))
		heights[index] = weekHeight(days[from:to], request.NormalizeResearchBlocks, request.TimeSlots)
	}
	...
	for index := 0; index < weekCount; index++ {
		from := index * 7
		to := min(from+7, len(days))
		renderWeek(canvas, image.Pt(30, y), width-60, days[from:to], faces, request.NormalizeResearchBlocks, request.ShowGroupNames, request.TimeSlots)
		y += heights[index] + weekGap
	}
	return canvas
}

func weekHeight(days []Day, normalizeResearch bool, reference ...[]domain.LessonTimeSlot) int {
	slots := weekSlots(days, normalizeResearch, reference...)
	if len(slots) == 0 {
		return 72 + 140
	}
	height := 72
	for _, slot := range slots {
		height += slotHeight(days, slot, normalizeResearch)
	}
	return height
}

func renderWeek(canvas *image.RGBA, origin image.Point, width int, days []Day, faces *faces, normalizeResearch bool, showGroupNames bool, reference ...[]domain.LessonTimeSlot) {
	const timeWidth = 145
	height := weekHeight(days, normalizeResearch, reference...)
	...
	y := panel.Min.Y + 72
	for _, slot := range slots {
		rowHeight := slotHeight(days, slot, normalizeResearch)
		...
		for dayIndex, day := range days {
			...
			lessons := lessonsAt(day.Lessons, slot, normalizeResearch)
			if len(lessons) == 0 {
				if label := emptySlotLabel(day.Lessons, slot, normalizeResearch); label != "" {
					drawCentered(canvas, faces.small, mutedColor, cell.Min.X, cell.Max.X, cell.Min.Y+rowHeight/2, label)
				}
				continue
			}
...
func weekSlots(days []Day, normalizeResearch bool, reference ...[]domain.LessonTimeSlot) []timeSlot {
	unique := map[timeSlot]struct{}{}
	for _, day := range days {
		for _, lesson := range day.Lessons {
			unique[visualTimeSlot(lesson, normalizeResearch)] = struct{}{}
		}
	}
	...
	var grid []domain.LessonTimeSlot
	if len(reference) > 0 {
		grid = reference[0]
	}
	return fillTimeGrid(result, grid)
}

func lessonsAt(lessons []domain.Lesson, slot timeSlot, normalizeResearch bool) []domain.Lesson {
	result := make([]domain.Lesson, 0, 2)
	for _, lesson := range lessons {
		if visualTimeSlot(lesson, normalizeResearch).start == slot.start {
...
func visualTimeSlot(lesson domain.Lesson, normalizeResearch bool) timeSlot {
	if normalizeResearch && lesson.TimeStart == "08:00" && isResearchWork(lesson.Subject) {
...
func slotHeight(days []Day, slot timeSlot, normalizeResearch bool) int {
	maximum := 1
	for _, day := range days {
		maximum = max(maximum, len(lessonsAt(day.Lessons, slot, normalizeResearch)))
	}
	return maximum * 140
}
